...
export default function QuestionsPage() {
  ...
  const [questions, setQuestions] = useState([
    { id: 1, text: 'What is 2 + 2?', options: ['3', '4', '5', '6'], correct: '4', explanation: '' },
  ])
  const [current, setCurrent] = useState(1)

  const add = () => {
    setQuestions([...questions, { id: Date.now(), text: '', options: ['', '', '', ''], correct: '', explanation: '' }])
  ...
  return (
    <div className="w-full">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={() => navigate('/tests/preview')} className={`${tokens.btnPrimary} px-4 py-2 text-sm`}>Publish</button>
      </div>

      <div className="grid md:grid-cols-[260px_1fr] gap-6">
        {/* Left sidebar list */}
        <aside className="bg-white border border-[#60A5FA]/40 rounded-xl p-4 h-fit">
          <h3 className="text-sm font-semibold text-[#374151] mb-3">Question List</h3>
          <div className="space-y-2">
            {questions.map((q, i) => (
              <button
                key={q.id}
                onClick={() => setCurrent(q.id)}
                className={`w-full text-left px-3 py-2 rounded-lg text-sm flex items-center gap-2 transition ${current === q.id ? 'bg-[#1B5DEF]/10 text-[#1B5DEF] font-medium' : 'bg-slate-50 text-[#374151] hover:bg-slate-100'}`}
              >
                <span className="w-5 h-5 rounded-full bg-[#1B5DEF] text-white text-[10px] flex items-center justify-center font-bold">{i + 1}</span>
                <span className="truncate">{q.text || `Question ${i + 1}`}</span>
              </button>
            ))}
          </div>
          <div className="mt-3 text-xs text-[#6B7280]">Total Questions : {questions.length}</div>
        </aside>
        ...
        <div>
          ...
          <div className={`${tokens.cardDark} p-6`}>
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex gap-2">
                <button onClick={() => add()} className={`${tokens.btnPrimary} text-xs px-3 py-1.5`}>+ MCQ</button>
                <button onClick={() => add()} className={`${tokens.btnSecondary} text-xs px-3 py-1.5`}>CSV</button>
              </div>
            </div>
              ...
              <div className="space-y-2">
                {currentQ?.options.map((opt: string, i: number) => (
                  <div key={i} className="flex items-center gap-3 bg-white border border-slate-300 rounded-lg px-3 py-2.5">
                    <input type="radio" name="correct" className="accent-[#1B5DEF]" defaultChecked={currentQ.correct === String.fromCharCode(65+i)} />
                    <input defaultValue={opt} className="bg-transparent text-sm w-full text-[#000A3A] placeholder:text-[#6B7280] focus:outline-none" placeholder="Type Option here" />
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="mb-4">
              ...
              <textarea
                defaultValue={currentQ?.explanation}
                placeholder="Type here"
            ...
            <div className="grid md:grid-cols-3 gap-3 mb-6">
              <div>
                ...
                <select className={`${tokens.input} text-sm`}><option>Select from Drop-down</option></select>
              </div>
              <div>
          ...
          <div className="flex justify-end gap-3 mt-6">
            ...
            <button onClick={() => navigate('/tests/preview')} className={`${tokens.btnPrimary} px-6 py-2.5 text-sm`}>Next</button>
          </div>
        </div>
      </div>
    </div>
